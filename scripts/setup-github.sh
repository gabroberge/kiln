...
REPO="${1:-gabroberge/turborepo-template}"
if [[ $# -le 1 ]]; then
	BRANCHES=(master dev)
else
	shift
...
# GitHub exposes one toggle for both creating and approving PRs
# (`can_approve_pull_request_reviews`). It must be true or GITHUB_TOKEN
# cannot open the Changesets version PR. Nothing in the workflows submits
# a review; github-actions[bot] also cannot approve a PR it opened.
echo "→ Setting Actions workflow token to read-only by default (PR create allowed, no auto-approve in workflows)…"
